...
set_message "Debug6" " Value of Base_Catalog_function : [ ${Base_Catalog_function} ]"
set_message "Debug6" " Value of New_Lib_Name          : [ ${New_Lib_Name} ]"
for New_Lib_Name in $(for Tvar in $( cat ../data/catalogue/function.cat | awk -F ";" '{ echo $2 }') ; do  basename ${Tvar} ; done | sort -u)
    do
      for function_geted in $( cat ${Base_Catalog_function} | grep ${New_Lib_Name} | awk -F ";" '{ echo $4 }' )
          do
             DPFMP_Base_File_name="$( echo ${function_geted} | awk -F "." '{ echo $1 }' )"
             ...
             cat ${Base_Catalog_function_docs} | grep "\;${function_geted}\;" |  awk -F ";" '{ echo "    " $6}' >> ${Document_doc_man}
             echo "</B>"                                                                                     >> ${Document_doc_man}
