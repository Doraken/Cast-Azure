...
for libs in ${Lib_to_SRC}
    do
       set_message "Info" "Found Library file : [ ${libs} ]"
       do_file_read "document_get_function_core" < ${libs}
done
document_get_deps
...
case ${Part_Item} in
             function) function_name="$( echo "${geted_item}" | awk '{ echo $2 }' )"
     	               ...
     	               ;;
                   \{) set_message "Debug5" "Start of function : [ ${function_name} ]"
                       ;;
                   \}) set_message "Debug5" "End of function : [ ${function_name} ]"
                       function_name=""
                       ;;
               \#\|\#) if [ ! -z "${function_name}" ]
                          then
                        ...
                        ;;
    do_empty_var_control)  DGFC_EMPT_CTRL_CALL="$(echo "${geted_item}" | awk '{ $6 }')"
                   ...
                   DGB_DEP="$(cat ${Base_Catalog_function} | grep "${DGD_Check_Item}$"  )"
         	            if [ -z "${DGB_DEP}" ]
         	               then
                               ...
                               set_message "Debug4" "function ${printable_function_name} Depend on : [ ${printable_Depend_On_Item} ] "
                               FONT_set_Basic_Font
...
do_file_read "autodoc_filter_line_Printer " < ${Base_Catalog_function_depend}
...
case ${TOPrintFormat} in
	XML|xml) set_message "Info" "Output format for documentation : [ ${TOPrintFormat} ] "
	         Document_Print_global_Print_xml
	         ;;
	TXT|txt) set_message "Info" "Output format for documentation : [ ${TOPrintFormat} ] "
	         Document_Print_global_Print_txt
	         ;;
	MANHTML|manhtml) set_message "Info" "Output format for documentation : [ ${TOPrintFormat} ] "
	         Document_Print_function_man_page
	         ;;
          *) set_message "ErrorN" "Unsupported Output format for documentation : [ ${TOPrintFormat} ] " "2"
             ;;
esac
