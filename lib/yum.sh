...
do_empty_var_control "${YumPackageToInstall}" "YumPackageToInstall" "ErrorN" "2" "package name is a mandatory parameter "
yum_check_Package ${YumPackageToInstall}
if [ "${YumPackageStatus}" = "NOT INSTALLED" ] 
then 
    yum install ${YumPackageToInstall} -y> /dev/null 2>&1 
    do_error_control "${?}" "status for package : [ ${YumPackageToCheck}] " "Not Installed" "4" "" "" "nomsg"
else
    set_message "Debug" "Package ${YumPackageStatus} : [ INSTALLED ] "
fi
...
then 
    ...
    do_error_control "${?}" "status for package : [ ${YumPackageToUnInstall}] " "Can t uninstall" "4" "" "" "nomsg"
else
    set_message "Debug" "Package ${YumPackageStatus} : [ NOT INSTALLED ] "
fi
...
do_empty_var_control "${YumPackageToCheck}" "YumPackageToCheck" "ErrorN" "2" "package name is a mandatory parameter "
yum list installed ${YumPackageToCheck} -q> /dev/null 2>&1
do_error_control "${?}" "status for package : [ ${YumPackageToCheck}] " "Not Installed" "0" "yum_check_Package_sub_u" "yum_check_Package_sub_i" "nomsg"
